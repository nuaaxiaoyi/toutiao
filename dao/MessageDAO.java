...
//            "(select conversation_id, max(id) as lastest_id ",
//            "from (select ", SELECT_FIELDS, " from ", TABLE_NAME, " where from_id=#{userId} or to_id=#{userId} order by id desc) as tt",
//            "group by conversation_id) as pp",
//            "where message.id = pp.lastest_id",
//            "order by id",
//             "limit #{offset},#{limit}"
//    })
    @Select({"select * from message join (\n" +
            "select max(id) as lastest_id \n" +
